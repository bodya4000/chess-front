...
class ChessHelper {
	...
	static getCastlingRookTargetCells(king: Figure): Cell[] {
		...
		const step = 2;
		return [board.getCell(kingCell.getRowPos(), kingCell.getColPos() + step), board.getCell(kingCell.getRowPos(), kingCell.getColPos() - step)];
	}
	 ...
	 */
	static getCellForCastlingRook(king: Figure, targetCell: Cell): Cell {
		const kingCell = king.getCell();
		const board = targetCell.getBoard();

		const isTargetLeft = targetCell.getColPos() < kingCell.getColPos();
		const rookColPos = isTargetLeft ? kingCell.getColPos() - 1 : kingCell.getColPos() + 1;

		return board.getCell(kingCell.getRowPos(), rookColPos);
	}
